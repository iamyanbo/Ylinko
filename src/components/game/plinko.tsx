    ...
    useEffect(() => {
        ...
        const createBottomCubes = (x: number, y: number, num: number, gap: number, width: number, mults: number[]) => {
            ...
            const updateText = () => {
                ...
                const distanceFromTop = (viewHeight - 800 + 140) / 2;
                const viewWidth = window.innerWidth;
                // If window size is less than 1080, don't display it
                if (viewWidth < 1280) {
                ...
                const distanceFromLeft = (viewWidth - 1000) / 2;
                cubes.forEach((cube, index) => {
                    const text = document.getElementById(`text-${index}`);
                    const pos = cube.position;
                    if (text) {
                        text.style.left = `${pos.x + distanceFromLeft}px`;
                        text.style.top = `${pos.y + distanceFromTop}px`;
            ...
            for (let i = 0; i < halfNum; i++) {
                ...
                text.id = `text-${i}`;
                text.textContent = "x" + mults[i].toString();
